    ...
    if (m == 0.0) {
        ...
        if (m > -1.0 && m < 1.0) {
            do
            {
                ...
                x += hdir;
                yf = m * ((float)x - x1) + y1;
                y = round_to_nearest(yf); // 0.5 will round up

            } while (x >= 0 && x < sb->xdots && x != x1 + hdir);
        } else {
            do {
                render_dot(sb, x, y);
                if (xf - x == -0.5) {
                ...
                y += vdir;
                xf = ((float)y - y1) / m + x1;
                x = round_to_nearest(xf); // 0.5 will round up
            } while (y >= 0 && y <= sb->ydots && y != y1 + vdir);
        }
    }
}
    ...
    for (int y = ymin; y != ymax + 1; y++) {
        ...
        for (size_t i = 0; i < nvertices; i++) {
            ...
            if (y == a.y && y == b.y) {
                ...
                scan_line(vt->cur_sb, CLAMP(a.x, 0, vt->xdots - 1), a.y, CLAMP(b.x, 0, vt->xdots - 1), b.y);
            } else if ((y == a.y || y == b.y) && (y == ymin || y == ymax)) {
                // Special case: x intercept is a min/max vertex
                int xcept = (y == a.y ? a.x : b.x);
                render_dot(vt->cur_sb, CLAMP(xcept, 0, vt->xdots - 1), y);
        ...
        assert(ncepts != 1);
        if (ncepts == 2) {
            // We know the line is horizontal, so we can do some simplified clipping.
            int x0 = CLAMP(xcepts[0], 0, vt->xdots - 1);
            int x1 = CLAMP(xcepts[1], 0, vt->xdots - 1);
            scan_line(vt->cur_sb, x0, y, x1, y);
        }
    }
